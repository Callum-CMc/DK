...
/**
 * Pub Quiz (Reusable Rounds) + Commit–Reveal + USDC entry/prize + ERC1155 NFTs (ON-CHAIN METADATA)
 *
 * Token IDs:
 *  - LOST_BASE   + roundId = LOST for that round (unique per round)
 *  - WINNER_BASE + roundId = WINNER for that round (unique per round; includes winning team name)
 *
 * Rounds:
 *  - Owner starts a new round with new answer hashes, fees, windows, salt.
 *  - Players commit during active round.
 *  - First correct reveal wins that round’s prize and Winner NFT.
 */
contract PubQuiz2 is ERC1155, Ownable, ReentrancyGuard {
    // ---------------------------
    // Custom errors
    // ---------------------------
    error ZeroAddress();
    ...
    // ---------------------------
    // Lost SVG (constant, pre-encoded to avoid any on-chain encoding)
    // Notes:
    //  - No spaces
    //  - '%' in SVG attributes is encoded as %25
    //  - This is a valid data:image/svg+xml URI
    // ---------------------------
string internal constant LOST_IMAGE_URI =
        "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='400' height='400'>"
        "<rect width='100%25' height='100%25' fill='black'/>"
        "<text x='50%25' y='50%25' dominant-baseline='middle' text-anchor='middle' fill='white' font-size='28'>"
        "Lost%20the%20Pub%20Quiz"
        "</text></svg>";

string internal constant WINNER_IMAGE_URI =
    "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='400' height='400'>"
    "<rect width='100%25' height='100%25' fill='black'/>"
    "<text x='50%25' y='50%25' dominant-baseline='middle' text-anchor='middle' fill='white' font-size='26'>"
    "Pub%20Quiz%20Winner"
    "</text></svg>";
    ...
    function commit(bytes32 commitHash, string calldata teamName) external nonReentrant {
        uint256 r = currentRound;
        if (r == 0) revert NoActiveRound();
        if (rounds[r].won) revert RoundAlreadyWon();
    ...
    function reveal(string[10] calldata answers, bytes32 userSalt) external nonReentrant {
        uint256 r = currentRound;
        if (r == 0) revert NoActiveRound();
        ...
        bytes32 answersHash = computeAnswersHash(answers);
    ...
    function clearExpiredCommit(uint256 roundId, address player) external {
        ...
        if (c.commitHash == bytes32(0)) revert NoCommit();
        if (c.revealed) revert AlreadyRevealed();
        if (!_isExpired(roundId, c.commitTime)) revert CommitExpired();
    ...
    function computeAnswersHash(string[10] calldata answers) public pure returns (bytes32) {
        bytes memory packed;
        for (uint256 i = 0; i < 10; i++) {
    ...
    function computeCommitHash(
        ...
        string[10] calldata answers,
        bytes32 userSalt
    ) external pure returns (bytes32) {
        bytes32 teamHash = keccak256(bytes(teamName));
        bytes32 answersHash = computeAnswersHash(answers);
        return keccak256(abi.encodePacked(player, teamHash, answersHash, userSalt));
    }
...
if (id >= LOST_BASE && id < LOST_BASE + currentRound + 1) {
    ...
    return string(
        abi.encodePacked(
            ...
            "{",
                "\"name\":\"Lost the Pub Quiz\",",
                "\"description\":\"Participation NFT for teams that did not win.\",",
                "\"image\":\"", LOST_IMAGE_URI, "\",",
                "\"attributes\":[",
                    "{\"trait_type\":\"Result\",\"value\":\"Failed\"},",
        ...
        // WINNER tokens: WINNER_BASE + roundId
...
return string(
    ...
    // ============================================================
    // make sure the round number is a string for the lost nft
    // ============================================================
